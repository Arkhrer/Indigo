...
RoomThreeSink::RoomThreeSink(): State(){
    this->justChanged = false;

    this->states[0] = GameData::stateVariables[23];
    ...
    if(!(GameData::stateVariables[23])){
        img = new Sprite(*backgroundGo, "Assets/Images/Room3/Sink/1.png");
    }
    else{
        img = new Sprite(*backgroundGo, "Assets/Images/Room3/Sink/2.png");
    }
...
RoomThreeSink::~RoomThreeSink(){

}

void RoomThreeSink::LoadAssets(){

}

void RoomThreeSink::Update(float dt){
    if(InputManager::GetInstance().KeyPress(ESCAPE_KEY)){
        Game::GetInstance().Push(new PauseMenu());
    }

    if(this->states[0] != GameData::stateVariables[23]){
        justChanged = true;
    }
    this->states[0] = GameData::stateVariables[23];
...
void RoomThreeSink::Render(){
    ...
    if(justChanged){
        ...
        if(!(GameData::stateVariables[23])){
            backgroundImg->Open("Assets/Images/Room3/Sink/1.png");
        }else{
            backgroundImg->Open("Assets/Images/Room3/Sink/2.png");
        }
...
void RoomThreeSink::Pause(){

}

void RoomThreeSink::Resume(){

}


void RoomThreeSink::SpawnFork(){
    GameObject* forkInteractionZoneGo = new GameObject();
    InteractionZone* forkInteractionZone = new InteractionZone(*forkInteractionZoneGo, 23);
    forkInteractionZoneGo->AddComponent(dynamic_cast<Component*>(forkInteractionZone));
    Collider* forkInteractionZoneCollider = new Collider(*forkInteractionZoneGo);
    forkInteractionZoneGo->AddComponent(dynamic_cast<Component*>(forkInteractionZoneCollider));
    AddObject(forkInteractionZoneGo);

    forkInteractionZoneGo->box.w = 150;
    forkInteractionZoneGo->box.h = 144;
    forkInteractionZoneGo->box.x = 582 + Camera::pos.x;
    forkInteractionZoneGo->box.y = 486 + Camera::pos.y;
}
